...
namespace AdventOfCode.Puzzles.Year2015
{
    ...
    /// </summary>
    [TestClass]
    public class Day06ProbablyAFireHazard
    {
        /// <summary>
        ...
        {
            Assert.AreEqual(998996, ExecutePart1(sampleInput1));
        }
        ...
        {
            Assert.AreEqual(377891, ExecutePart1(puzzleInput));
        }
        ...
        {
            Assert.AreEqual(2000001, ExecutePart2(sampleInput2));
        }
        ...
        {
            Assert.AreEqual(14110788, ExecutePart2(puzzleInput));
        }

        /// <summary>
        /// Executes part 1 of the puzzle
        /// </summary>
        /// <param name="instructions">The lighting pattern instructions</param>
        /// <returns>The number of lights on at the end</returns>
        private static int ExecutePart1(string[] instructions)
        {
            bool[,] grid = new bool[1000, 1000];
            foreach (string instruction in instructions)
            {
                (int startX, int startY, int endX, int endY) = GetIndices(instruction);
                if (instruction.StartsWith("turn on"))
                {
                    Set(ref grid, true, startX, startY, endX, endY);
                }
                else if (instruction.StartsWith("turn off"))
                {
                    Set(ref grid, false, startX, startY, endX, endY);
                }
                else if (instruction.StartsWith("toggle"))
                {
                    Toggle(ref grid, startX, startY, endX, endY);
                }
            }
        ...
        /// </summary>
        /// <param name="instructions">The lighting pattern instructions</param>
        /// <returns>The total brightness of the lights at the end</returns>
        private static int ExecutePart2(string[] instructions)
        {
            ...
            foreach (string instruction in instructions)
            {
                ...
                if (instruction.StartsWith("turn on"))
                {
                    Increment(ref grid, 1, startX, startY, endX, endY);
                }
                else if (instruction.StartsWith("turn off"))
                {
                    Increment(ref grid, -1, startX, startY, endX, endY);
                ...
                {
                    Increment(ref grid, 2, startX, startY, endX, endY);
                }
            }
        ...
        /// <summary>
        /// Gets the start and end indices from the instruction
        /// </summary>
        /// <param name="instruction">The instruction to parse</param>
        /// <returns>The start and end indices</returns>
        private static (int startX, int startY, int endX, int endY) GetIndices(string instruction)
        {
            var match = indexRegex.Match(instruction);
            Assert.IsTrue(match.Success, "Instruction did not match the format");
            return (int.Parse(match.Groups[1].Value), int.Parse(match.Groups[2].Value),
                    int.Parse(match.Groups[3].Value), int.Parse(match.Groups[4].Value));
        }
        private static readonly Regex indexRegex = new Regex("(?:turn on|turn off|toggle) ([0-9]*),([0-9]*) through ([0-9]*),([0-9]*)");

        /// <summary>
        /// Sets all values in the specified range to the given value
        /// </summary>
        ...
        /// <param name="endX">The ending X index</param>
        /// <param name="endY">The ending Y index</param>
        private static void Set(ref bool[,] grid, bool value, int startX, int startY, int endX, int endY)
        {
        ...
        /// <summary>
        /// Toggles the values in the specified range
        /// </summary>
        /// <param name="grid">The grid of lights</param>
        /// <param name="startX">The starting X index</param>
        /// <param name="startY">The starting Y index</param>
        /// <param name="endX">The ending X index</param>
        /// <param name="endY">The ending Y index</param>
        private static void Toggle(ref bool[,] grid, int startX, int startY, int endX, int endY)
        {
            for (int i = startX; i <= endX; i++)
            {
                for (int j = startY; j <= endY; j++)
        ...
        /// <summary>
        /// Applies an incremental change to all values in the specified range
        /// </summary>
        /// <param name="grid">The grid of lights</param>
        ...
        /// <param name="endX">The ending X index</param>
        /// <param name="endY">The ending Y index</param>
        private static void Increment(ref int[,] grid, int amount, int startX, int startY, int endX, int endY)
        {
            for (int i = startX; i <= endX; i++)
            {
                for (int j = startY; j <= endY; j++)
                {
                    grid[i, j] = Math.Max(grid[i, j] + amount, 0); //Min of 0
                }
            }
        }
