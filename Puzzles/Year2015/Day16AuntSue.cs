...
namespace AdventOfCode.Puzzles.Year2015
{
    ...
    public class Day16AuntSue
    {
        ...
        {
            // There is no sample input for this puzzle
        }
        ...
        {
            Assert.AreEqual(213, ExecutePart1(puzzleInput));
        }
        ...
        {
            Assert.AreEqual(323, ExecutePart2(puzzleInput));
        }

        /// <summary>
        /// Executes part 1 of the puzzle
        /// </summary>
        /// <param name="aunts">The list of compounds for each aunt</param>
        /// <returns>The number of the aunt that meets the criteria</returns>
        private static int ExecutePart1(string[] aunts)
        {
            // Use simple equality for all compounds
            return FindAunt(aunts, _ => (a, b) => a == b);
        }
        ...
        /// <param name="aunts">The list of compounds for each aunt</param>
        /// <returns>The number of the aunt that meets the criteria</returns>
        private static int ExecutePart2(string[] aunts)
        {
            // Look up the comparison function to use for the compound
            return FindAunt(aunts, compound => targetCompoundComparisons[compound]);
        }

        /// <summary>
        /// Finds the aunt that matches the target compounds
        /// </summary>
        /// <param name="aunts">The list of compounds for each aunt</param>
        /// <param name="getComparerFunc">The function that specifies what value to compare 
        /// against a given component and the function to use to compare the values</param>
        /// <returns>The number of the aunt that meets the criteria</returns>
        private static int FindAunt(string[] aunts, 
            Func<string, Func<int, int, bool>> getComparerFunc)
        {
            foreach (string aunt in aunts)
            {
                var (number, compounds) = ParseAunt(aunt);
                bool invalidFound = false;
                foreach (var compound in compounds)
                {
                    // If any of the known values do not match, go on to the next aunt
                    var comparer = getComparerFunc(compound.Key);
                    if (!comparer(compound.Value, targetCompounds[compound.Key]))
                    {
                        invalidFound = true;
                        break;
                    }
        ...
        /// <summary>
        /// Parses the compound values for an aunt
        /// </summary>
        /// <param name="aunt">The aunt to parse</param>
        /// <returns>The aunt number and compound values</returns>
        private static (int number, Dictionary<string, int> compounds) ParseAunt(string aunt)
        {
            string[] parts = aunt.Split(':', ',');
            int number = int.Parse(parts[0][4..]);
            var compounds = new Dictionary<string, int>();
            for (int i = 1; i < parts.Length - 1; i += 2)
            {
                compounds[parts[i].Trim()] = int.Parse(parts[i + 1].Trim());
            }
        ...
        private static readonly Dictionary<string, int> targetCompounds =
        ...
        private static readonly Dictionary<string, Func<int, int, bool>> 
            targetCompoundComparisons = new Dictionary<string, Func<int, int, bool>>()
            {
                ["children"]    = (a, b) => a == b,
                ["cats"]        = (a, b) => a > b,
                ["samoyeds"]    = (a, b) => a == b,
