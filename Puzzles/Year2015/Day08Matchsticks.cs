...
namespace AdventOfCode.Puzzles.Year2015
{
    ...
    public class Day08Matchsticks
    {
        ...
        {
            Assert.AreEqual(12, ExecutePart1(sampleInput));
        }
        ...
        {
            Assert.AreEqual(1333, ExecutePart1(puzzleInput));
        }
        ...
        {
            Assert.AreEqual(19, ExecutePart2(sampleInput));
        }
        ...
        {
            Assert.AreEqual(2046, ExecutePart2(puzzleInput));
        }
        ...
        /// <returns>The number of characters of code for string literals minus the number of 
        /// characters in memory for the values of the strings</returns>
        private static int ExecutePart1(string[] list)
        {
            int count = 0;

            foreach (string line in list)
            {
                count += 2; //begin and end quotes
                count += singleRegex.Matches(line).Count; //escaped " and /
                count += 3 * tripleRegex.Matches(line).Count; //escaped hex codes
            }
        ...
        /// </summary>
        /// <param name="list">The list of strings</param>
        /// <returns>The total number of characters to represent the newly encoded strings minus 
        /// the number of characters of code in each original string literal</returns>
        private static int ExecutePart2(string[] list)
        {
            int count = 0;

            foreach (string line in list)
            {
                string recodedLine = line.Replace("\\", "\\\\").Replace("\"", "\\\"");

                count += 4; //begin and end quotes
                count += singleRegex.Matches(recodedLine).Count; //escaped " and \
            }

            return count;
        }

        private static readonly Regex singleRegex = new Regex(@"(\\""|\\\\)");
        private static readonly Regex tripleRegex = new Regex(@"(\\x[a-f0-9]{2})");
