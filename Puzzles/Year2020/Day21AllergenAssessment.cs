...
namespace AdventOfCode.Puzzles.Year2020
{
    ...
    public class Day21AllergenAssessment
    {
        ...
        {
            Assert.AreEqual(2542, ExecutePart1(puzzleInput));
        }
        ...
        {
            Assert.AreEqual("mxmxvkd,sqjhc,fvjkl", ExecutePart2(sampleInput));
        }
        ...
        {
            Assert.AreEqual("hkflr,ctmcqjf,bfrq,srxphcm,snmxl,zvx,bd,mqvk", ExecutePart2(puzzleInput));
        }

        /// <summary>
        /// Executes part 1 of the puzzle
        /// </summary>
        /// <param name="foodDefinitionList">The list of food ingredients and allergens</param>
        /// <returns>The number of times a non-allergen ingredient is found</returns>
        private static int ExecutePart1(string[] foodDefinitionList)
        {
            var (foods, _) = Execute(foodDefinitionList);

            // We should be left only with safe ingredients
        ...
        /// </summary>
        /// <param name="foodDefinitionList">The list of food ingredients and allergens</param>
        /// <returns>The cannonical dangerous ingredient list</returns>
        private static string ExecutePart2(string[] foodDefinitionList)
        {
            var(_, dangerousIngredients) = Execute(foodDefinitionList);

            return string.Join(',', dangerousIngredients.Values);
        }

        private static (List<Food>, SortedList<string, string>) Execute(string[] foodDefinitionList)
        {
            var (foods, allergens) = Initialize(foodDefinitionList);

            // List of ingredients sorted by their allergen
            var dangerousIngredients = new SortedList<string, string>();

            while (allergens.Any())
            {
                // Make a copy of the allergen list to iterate through so we can make modifications
                // to the original
                var allergensCopy = new HashSet<string>(allergens);
                foreach (string allergen in allergensCopy)
                {
                    // Get the list of all foods that contain this known allergen
                    var foodsWithAllergen = foods.Where(f => f.Allergens.Contains(allergen));

                    // Build the list of common ingredients of the foods that contain the allergen
                    ...
                    // If there is only one ingredient left, it must be the one that contains
                    // the allergen
                    if (intersection.Count() == 1)
                    {
                        ...
                        // Remove the allergen and ingredient from the list
            ...
            return (foods, dangerousIngredients);
        }

        /// <summary>
        /// A food with its ingredients and known allergens
        /// </summary>
        private record Food
        {
            /// <summary>
            /// The known allergens for this food
            /// </summary>
            public List<string> Allergens { get; } = new List<string>();

            /// <summary>
            /// The ingredients for this food
            /// </summary>
            public List<string> Ingredients { get; } = new List<string>();
        }

        /// <summary>
        /// Parses the food definitions into a set of foods with their ingredients and a list of
        /// all allergens
        /// </summary>
        /// <param name="foodDefinitionList">The list of food ingredients and allergens</param>
        /// <returns>The list of foods and allergens</returns>
        private static (List<Food> foods, HashSet<string> allergens) Initialize(
            string[] foodDefinitionList)
        {
