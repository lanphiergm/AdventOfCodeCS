...
namespace AdventOfCode.Puzzles.Year2020
{
    ...
    /// </summary>
    [TestClass]
    public class Day04PassportProcessing
    {
        /// <summary>
        ...
        {
            Assert.AreEqual(247, ExecutePart1(puzzleInput));
        }
        ...
        {
            Assert.AreEqual(4, ExecutePart2(sampleInput2));
        }
        ...
        {
            Assert.AreEqual(145, ExecutePart2(puzzleInput));
        }

        /// <summary>
        /// Executes part 1 of the puzzle
        /// </summary>
        /// <param name="data">The passport data</param>
        /// <returns>The number of valid passports</returns>
        private static int ExecutePart1(string[] data)
        {
            return CountValidRecords(data, r => IsValid(r));
        }
        ...
        {
            return CountValidRecords(data, r => IsValidStrict(r));
        }
        ...
        /// </summary>
        /// <param name="data">The passport data</param>
        /// <param name="recordValidator">The function to use to determine if a passport is valid
        /// </param>
        /// <returns>The number of valid passports</returns>
        private static int CountValidRecords(string[] data, Func<string, bool> recordValidator)
        {
            int validCount = 0;
            string currRecord = string.Empty;
            for (int i = 0; i <= data.Length; i++)
            {
                if (i == data.Length || string.IsNullOrEmpty(data[i]))
                {
                    if (recordValidator(currRecord))
                    {
                        validCount++;
                    }
                    currRecord = string.Empty;
                }
                else
                {
                    currRecord += " " + data[i];
        ...
        /// <summary>
        /// Whether or not a specified passport record contains all the required fields
        /// </summary>
        /// <param name="record">The record to test</param>
        /// <returns>true if the record is valid; otherwise, false</returns>
        private static bool IsValid(string record)
        {
        ...
        /// <summary>
        /// Whether or not a specified passport record contains all the required fields and those
        /// fields contain valid data
        /// </summary>
        /// <param name="record">The record to test</param>
        /// <returns>true if the record is strictly valid; otherwise, false</returns>
        private static bool IsValidStrict(string record)
        {
            if (!IsValid(record))
            {
                return false;
        ...
        /// </summary>
        /// <param name="field">The field name and data</param>
        /// <returns>true if the field value is valid; otherwise, false</returns>
        private static bool ValidateField(string field)
        {
            string[] parts = field.Split(':');
            return GetValidator(parts[0]).Invoke(parts[1]);
        }

        /// <summary>
        /// Delegate for a method that validates a field value
        /// </summary>
        /// <param name="fieldValue">The value to validate</param>
        /// <returns>true if the field value is valid; otherwise, false</returns>
        private delegate bool FieldValidator(string fieldValue);

        /// <summary>
        /// Retrieves the validator delegate for the specified field name
        /// </summary>
        /// <param name="fieldName">The name of the field</param>
        /// <returns>The validator method if the field is found; otherwise, null</returns>
        private static FieldValidator GetValidator(string fieldName)
        {
            ...
            switch (fieldName)
            {
                case "byr":
                    ...
                    break;
                case "iyr":
                    validator = ValidateIssueYear;
                    break;
                case "eyr":
                    validator = ValidateExpirationYear;
                    break;
                case "hgt":
                    validator = ValidateHeight;
                    break;
                case "hcl":
                    validator = ValidateHairColor;
                    break;
                case "ecl":
                    validator = ValidateEyeColor;
                    break;
                case "pid":
                    validator = ValidatePassportId;
                    break;
                case "cid":
        ...
        /// <param name="minInclusive">The minimum (inclusive) valid year value</param>
        /// <param name="maxInclusive">The maximum (inclusive) valid year value</param>
        /// <returns>true if the field value falls in the specified range; otherwise, 
        /// false</returns>
        private static bool ValidateYear(string fieldValue, int minInclusive, int maxInclusive)
        {
            return int.TryParse(fieldValue, out int year) &&
                year >= minInclusive && year <= maxInclusive;
        }
        ...
        /// </summary>
        /// <param name="fieldValue">The field value</param>
        /// <returns>true if the field value is between 1920 and 2002 (inclusive); otherwise,
        /// false</returns>
        private static bool ValidateBirthYear(string fieldValue)
        {
            return ValidateYear(fieldValue, 1920, 2002);
        }
        ...
        /// </summary>
        /// <param name="fieldValue">The field value</param>
        /// <returns>true if the field value is between 2010 and 2020 (inclusive); otherwise,
        /// false</returns>
        private static bool ValidateIssueYear(string fieldValue)
        {
            return ValidateYear(fieldValue, 2010, 2020);
        }
        ...
        /// </summary>
        /// <param name="fieldValue">The field value</param>
        /// <returns>true if the field value is between 2020 and 2030 (inclusive); otherwise,
        /// false</returns>
        private static bool ValidateExpirationYear(string fieldValue)
        {
            return ValidateYear(fieldValue, 2020, 2030);
        }
        ...
        /// </summary>
        /// <param name="fieldValue">The field value</param>
        /// <returns>true if the field value is between 150 and 193 cm (inclusive) or between
        /// 59 and 76 in (inclusive); otherwise, false</returns>
        private static bool ValidateHeight(string fieldValue)
        {
            var match = heightRegex.Match(fieldValue);
            if (match.Success)
            {
                int height = int.Parse(match.Groups[1].Value);
                if (match.Groups[2].Value == "cm" && height >= 150 && height <= 193)
                {
                    return true;
                }
                else if (match.Groups[2].Value == "in" && height >= 59 && height <= 76)
                {
                    return true;
                }
            }
        ...
        private static readonly Regex heightRegex = new Regex("^([0-9]*)(cm|in)$");

        /// <summary>
        /// Validates the hair color field
        /// </summary>
        /// <param name="fieldValue">The field value</param>
        /// <returns>true if the field value is a valid hex color; otherwise, false</returns>
        private static bool ValidateHairColor(string fieldValue)
        {
            return hairColorRegex.IsMatch(fieldValue);
        }
        ...
        /// </summary>
        /// <param name="fieldValue">The field value</param>
        /// <returns>true if the field value is in the list of valid values; otherwise, 
        /// false</returns>
        private static bool ValidateEyeColor(string fieldValue)
        {
            return eyeColorRegex.IsMatch(fieldValue);
        }
        private static readonly Regex eyeColorRegex = 
            new Regex("^(?:amb|blu|brn|gry|grn|hzl|oth)$");

        /// <summary>
        /// Validates the passport ID field
        /// </summary>
        /// <param name="fieldValue">The field value</param>
        /// <returns>true if the passport ID is a 9-digit number; otherwise, false</returns>
        private static bool ValidatePassportId(string fieldValue)
        {
            return passportIdRegex.IsMatch(fieldValue);
        }
        private static readonly Regex passportIdRegex = new Regex("^[0-9]{9}$");

        /// <summary>
        /// Validates the country ID field
        /// </summary>
        /// <param name="_">The field value</param>
        /// <returns>Always returns true since this is an optional field</returns>
        private static bool ValidateCountryId(string _) => true;
