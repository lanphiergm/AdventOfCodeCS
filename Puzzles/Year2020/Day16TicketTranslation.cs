...
namespace AdventOfCode.Puzzles.Year2020
{
    ...
    public class Day16TicketTranslation
    {
        ...
        {
            Assert.AreEqual(71L, ExecutePart1(sampleFieldDefinitions, 
                sampleNearbyTicketDefinitions));
        }
        ...
        {
            Assert.AreEqual(25895L, ExecutePart1(puzzleFieldDefinitions, 
                puzzleNearbyTicketDefinitions));
        }
        ...
        {
            // No sample answer available for part 2
        }
        ...
        {
            Assert.AreEqual(5865723727753, ExecutePart2(puzzleFieldDefinitions, puzzleMyTicketDefinition,
                puzzleNearbyTicketDefinitions));
        }
        ...
        /// <param name="nearbyTicketDefinitions">The nearby ticket definitions</param>
        /// <returns>The scanning error rate</returns>
        private static long ExecutePart1(string[] fieldDefinitions, string[] nearbyTicketDefinitions)
        {
            var fields = ParseFields(fieldDefinitions);
            var nearbyTickets = ParseNearbyTickets(nearbyTicketDefinitions);
            ...
            foreach (Ticket ticket in nearbyTickets)
            {
                ...
                {
                    if (!fields.Any(f => f.IsValid(value)))
                    {
        ...
        /// </summary>
        /// <param name="fieldDefinitions">The field definitions</param>
        /// <param name="myTicketDefinition">My ticket definition</param>
        /// <param name="nearbyTicketDefinitions">The nearby ticket definitions</param>
        /// <returns>The product of the departure field values</returns>
        private static long ExecutePart2(string[] fieldDefinitions, string myTicketDefinition,
            string[] nearbyTicketDefinitions)
        {
            var fields = ParseFields(fieldDefinitions);
            var nearbyTickets = ParseNearbyTickets(nearbyTicketDefinitions);
            ...
            // Using only the valid tickets, we can determine the field order
            DetermineFieldPositions(fields, validTickets);

            // Compute the product of the departure fields from my ticket
            var myTicket = new Ticket(myTicketDefinition);
            long product = 1;
            foreach (var field in fields.Where(f => f.Name.StartsWith("departure")))
            {
                product *= myTicket.FieldValues[field.Position];
            }
            return product;
        }

        /// <summary>
        /// Determines the field positions based on valid values
        /// </summary>
        /// <param name="fields">The list of fields</param>
        /// <param name="tickets">The list of valid tickets</param>
        private static void DetermineFieldPositions(List<Field> fields, List<Ticket> tickets)
        {
            // Start with a list of all available positions
            var availablePositions = new List<int>();
            for (int i = 0; i < tickets[0].FieldValues.Count; i++)
            {
            ...
            while (availablePositions.Any())
            {
                ...
                {
                    // Collect the list of all positions where every ticket is valid for this field
                    var validPositions = new List<int>();
                    foreach (int position in availablePositions)
                    {
                        if (tickets.All(t => field.IsValid(t.FieldValues[position])))
                        {
                            validPositions.Add(position);
                        }
                    }

                    // If there is only one position that is valid for this field, claim it
                    if (validPositions.Count == 1)
                    {
                        field.Position = validPositions[0];
                        availablePositions.Remove(validPositions[0]);
                    }
                }
            }
        }
        ...
        /// <summary>
        /// Contains the validity rules for a field
        /// </summary>
        private class Field
        {
            ...
            /// Constructor
            /// </summary>
            /// <param name="definition">The definition for this field</param>
            ...
            /// <summary>
            /// Whether or not the specified value is valid for this field
            /// </summary>
            ...
            private static readonly Regex definitionRegex = new Regex(@"(.*): (\d+)-(\d+) or (\d+)-(\d+)");
        }

        /// <summary>
        /// Contains the field values for a ticket
        /// </summary>
        private class Ticket
        {
            ...
            /// Constructor
            /// </summary>
            /// <param name="definition">The definition for this ticket</param>
            public Ticket(string definition)
            {
                var values = definition.Split(',');
                foreach (string value in values)
                {
                    FieldValues.Add(int.Parse(value));
                }
            }

            /// <summary>
            /// The field values for this ticket
            /// </summary>
            public List<int> FieldValues { get; } = new List<int>();
        }

        /// <summary>
        /// Parses the field definitions
        /// </summary>
        /// <param name="fieldDefinitions">The field definitions</param>
        /// <returns>The list of fields</returns>
        private static List<Field> ParseFields(string[] fieldDefinitions)
        {
            var fields = new List<Field>();
            foreach (string def in fieldDefinitions)
            {
        ...
        /// <summary>
        /// Parses the nearby ticket definitions
        /// </summary>
        /// <param name="nearbyTicketDefinitions">The nearby ticket definitions</param>
        /// <returns>The list of tickets</returns>
        private static List<Ticket> ParseNearbyTickets(string[] nearbyTicketDefinitions)
        {
            var tickets = new List<Ticket>();
            foreach (string def in nearbyTicketDefinitions)
            {
                tickets.Add(new Ticket(def));
            }
            return tickets;
        }
        ...
        private static readonly string[] sampleFieldDefinitions =
        {
            "class: 1-3 or 5-7",
            "row: 6-11 or 33-44",
            "seat: 13-40 or 45-50"
        ...
        private static readonly string[] sampleNearbyTicketDefinitions =
        {
            "7,3,47",
            "40,4,50",
            "55,2,20",
        ...
        private static readonly string[] puzzleFieldDefinitions =
        {
        ...
        private const string puzzleMyTicketDefinition = "61,151,137,191,59,163,89,83,71,179,67," +
            "149,197,167,181,173,53,139,193,157";

        private static readonly string[] puzzleNearbyTicketDefinitions =
        {
