...
namespace AdventOfCode.Puzzles.Year2020
{
    ...
    public class Day14DockingData
    {
        ...
        {
            Assert.AreEqual(165L, ExecutePart1(sampleInput1));
        }
        ...
        {
            Assert.AreEqual(10050490168421L, ExecutePart1(puzzleInput));
        }
        ...
        {
            Assert.AreEqual(208L, ExecutePart2(sampleInput2));
        }
        ...
        {
            Assert.AreEqual(2173858456958L, ExecutePart2(puzzleInput));
        }

        /// <summary>
        /// Executes part 1 of the puzzle
        /// </summary>
        /// <param name="instructions">The initialization program instructions</param>
        /// <returns>The sum of the memory values</returns>
        private static long ExecutePart1(string[] instructions)
        {
            ...
            foreach (string instruction in instructions)
            {
                ...
                {
                    var (address, value) = ParseMemoryInstruction(instruction);
        ...
        /// <param name="instructions">The initialization program instructions</param>
        /// <returns>The sum of the memory values</returns>
        private static long ExecutePart2(string[] instructions)
        {
            var memory = new Dictionary<long, long>();
            string mask = null;
            foreach (string instruction in instructions)
            {
                ...
                else
                {
                    ...
                    foreach (long maskedAddress in addresses)
                    {
        ...
        private static long ApplyValueMask(long value, string mask)
        {
        ...
        /// <param name="mask">The mask to apply</param>
        /// <returns>The list of all addresses that fit the mask</returns>
        private static List<long> ApplyAddressMask(long address, string mask)
        {
            string originalAddressString = Convert.ToString(address, 2).PadLeft(36, '0');
            var builder = new StringBuilder();
            for (int i = 0; i < mask.Length; i++)
            {
                if (mask[i] == '0')
                {
                    builder.Append(originalAddressString[i]);
                }
                else if (mask[i] == '1')
                {
                    builder.Append('1');
            ...
            return PermuteAddresses(builder.ToString());
        }

        /// <summary>
        /// Build the list of all masked addresses
        /// </summary>
        /// <param name="maskedAddressString"></param>
        /// <returns>The list of all addresses</returns>
        private static List<long> PermuteAddresses(string maskedAddressString)
        {
            var addresses = new List<long>();
            int maskIndex = maskedAddressString.IndexOf('X');
            if (maskIndex == -1)
            {
                addresses.Add(Convert.ToInt64(maskedAddressString, 2));
            }
            else
            {
                string before = maskIndex == 0 ? string.Empty : maskedAddressString[0..maskIndex];
                string after = maskIndex == maskedAddressString.Length - 1 ? string.Empty : 
                    maskedAddressString[(maskIndex + 1)..];
                addresses.AddRange(PermuteAddresses(before + '0' + after));
                addresses.AddRange(PermuteAddresses(before + '1' + after));
            }
            return addresses;
        }

        /// <summary>
        /// Parses a memory instruction into its address and value
        /// </summary>
        /// <param name="instruction">The instruction to parse</param>
        /// <returns>The address and value</returns>
        private static (long address, long value) ParseMemoryInstruction(string instruction)
        {
