...
{
    class Day16AuntSue : ProblemBase<int>
    {
        public Day16AuntSue(ILogger logger) : base(logger, "Aunt Sue", 2015, 16) { }
        ...
        protected override int ExecutePart1()
        {
            foreach (string aunt in aunts)
            {
                ...
                {
                    if (targetCompounds[compound.Key] != compound.Value)
                    {
                        invalidFound = true;
                        break;
                    }
        ...
        private static readonly Dictionary<string, (int target, Func<int, int, bool> comparer)> targetCompoundComparisons =
            new Dictionary<string, (int target, Func<int, int, bool> comparer)>()
            {
                ["children"] = (3, (a, b) => a == b),
                ["cats"] = (7, (a, b) => a > b),
                ["samoyeds"] = (2, (a, b) => a == b),
        ...
        protected override int ExecutePart2()
        {
            foreach (string aunt in aunts)
            {
                ...
                foreach (var compound in compounds)
                {
                    var (target, comparer) = targetCompoundComparisons[compound.Key];
                    if (!comparer(compound.Value, target))
                    {
                        invalidFound = true;
                        break;
                    }
        ...
        private static (int number, Dictionary<string, int> compounds) ParseAunt(string aunt)
        {
            ...
            int number = int.Parse(parts[0].Substring(4));
        ...
        private static readonly string[] aunts =
        {
