...
{
    class Day06ProbablyAFireHazard : ProblemBase<int>
    {
        public Day06ProbablyAFireHazard(ILogger logger) : base(logger, "Probably a Fire Hazard", 2015, 6) { }
        ...
        private readonly Regex indexRegex = new Regex("(?:turn on|turn off|toggle) ([0-9]*),([0-9]*) through ([0-9]*),([0-9]*)");

        private (int startX, int startY, int endX, int endY) GetIndices(string instruction)
        {
            var match = indexRegex.Match(instruction);
            if (!match.Success)
            {
                Debug.Fail("Instruction did not match the format");
            }
        ...
        private static void Increment(ref int[,] grid, int amount, int startX, int startY, int endX, int endY)
        {
            for (int i = startX; i <= endX; i++)
            {
                for (int j = startY; j <= endY; j++)
                {
                    grid[i, j] = Math.Max(grid[i, j] + amount, 0);
                }
            }
        }
